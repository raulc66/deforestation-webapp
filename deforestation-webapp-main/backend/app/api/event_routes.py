...
from app.api.deps import deny_demo_global_data, forest_event_service_dep
...
async def events_for_map(
    ...
    """Generic event retrieval for map visualisation (unscoped).

    Returns canonical marker fields for all stored forest events matching the
    query limit — **not** filtered by ``GEOGRAPHIC_SCOPE``.  Intelligence
    dashboards must use ``GET /analytics/intelligence/map-overlay`` for
    scope-aware markers aligned with configured geographic policy.
    """
    events = await svc.list_events(limit=limit)
    return {
        "events": [forest_event_map_marker(e) for e in events],
    }
